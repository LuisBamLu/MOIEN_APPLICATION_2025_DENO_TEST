<script>
    ...
    /** @type {{profile?: any, profileArray?: any, userId?: any}} */
    let { profile = $bindable({}), profileArray = [], userId = $bindable(null) } = $props();
...
<dialog class:is-open={ isModalOpen }>
    <div class="modal">
        ...
        <div class="modal-body">
            {#if isArray( profileArray ) }
                {#each profileArray as currentProfile }
                    <div
                        class="modal-body-row"
                        class:is-selected={ profile?.userId === currentProfile.userId }
                        role="button"
                        tabindex="0"
                        onkeydown={() => handleProfileSelect( currentProfile )}
                        onclick={() => handleProfileSelect( currentProfile )}
                    >
                        ...
                        <div class="modal-body-row-photo">
                            <img src={ getStorageImagePath( currentProfile.imagePath, 640 ) } alt={ currentProfile.firstName }>
                        </div>
                    </div>
                {/each}
            {/if}
        ...
        <div class="modal-footer">
            <button type="button" onclick={handleProfileSelect}>Close</button>
        </div>
    </div>
</dialog>
<div
    class="ticket-profile-select"
    role="button"
    tabindex="0"
