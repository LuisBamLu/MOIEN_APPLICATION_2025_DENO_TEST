<script>
    ...
    import { hostRatings } from '$src/lib/store/ratingsStore';
    ...
    let hostName = hostData.firstName;
    let profileReviews = [];
    ...
    let isRatingButtonClicked =
        {
            hostOverall: false,
            hostCommunication: false,
            hostCheckIn: false,
            hostCompliance: false
        }
    let isSubmitButtonLoading = false;

    let unsubscribe = userBookingsStore.subscribe(
        store =>
        {
        ...
        )
    {
        if ( activeStep === 1 )
        {
            if ( !isRatingButtonClicked.hostOverall )
            {
    ...
    {
        let ratingValues = Object.values( $hostRatings );
        allRatingValues = [];
        ...
        let hostRatingsByReviewArray = hostData.reviewArray.map( review => review.rating );
        hostRatingsByReviewArray.push( average );

        updatedProfileRating = hostRatingsByReviewArray.reduce( ( a, b ) => a + b ) / hostRatingsByReviewArray.length;
        ...
        )
    {
        ...
        formData.append( 'ratedUserProfileId', hostId )
        ...
        try
        {
            ...
            if ( response.ok )
            {
                ...
                userBookingsStore.update(
                    currentBookings =>
                    {
                        currentBookings.propertyArrayByRentalBooking.map(
                            rental =>
                            {
                                if ( rental.rentalBooking.profile.userId === hostId )
                                {
                                    rental.rentalBooking.profile.reviewArray.push( newReview )
                                }
                            }
                            );
        ...
        <ModalContent>
            <div class="content-container">
                <div class="general-experience">
                    ...
                    <p class="text-center">
                        { getLocalizedTextBySlug( 'rate-your-experience-label', { hostName: hostName }, $languageTagStore ) }
                    </p>
                </div>
                <RatingButtons
                    { ratingType }
                    criteria="hostOverall"
        ...
        <ModalContent>
            <div class="content-container">
                ...
                <div class="criteria-container">
                    <RatingCriteria criteria="hostCommunication" />
                    <RatingButtons
                        { ratingType }
                        criteria="hostCommunication"
                        onRatingClick={ updateRatingStore }
                    />
                </div>
                <div class="criteria-container">
                    <RatingCriteria criteria="hostCheckIn" />
                    <RatingButtons
                        { ratingType }
                        criteria="hostCheckIn"
                        onRatingClick={ updateRatingStore }
                     />
                </div>
                <div class="criteria-container">
                    <RatingCriteria criteria="hostCompliance" />
                    <RatingButtons
                        { ratingType }
                        criteria="hostCompliance"
                        onRatingClick={ updateRatingStore }
                    />
                </div>
            </div>
        ...
        <ModalContent>
            ...
            <div class="content-container">
                <div class="review-header-container">
                    <img src="/image/supporting-documents/heading.svg" alt="">
                    <h1 class="public-comment-title">
                        { getLocalizedTextBySlug( 'experience-with-user-label', { hostName: hostName }, $languageTagStore ) }
                    </h1>
                </div>
                <ul class="review-list">
                    <li>
                        ...
                        <div class="average-item-rating">
                            {#if $hostRatings.hostOverall.rating }
                                <p>{ $hostRatings.hostOverall.rating }</p>
                                <img src={ $hostRatings.hostOverall.img } alt="Rating">
                            {:else}
                                <p>N/A</p>
                            {/if}
                        </div>
                    ...
                    <li>
                        ...
                        <div class="average-item-rating">
                            {#if $hostRatings.hostCommunication.rating }
                                <p>{ $hostRatings.hostCommunication.rating }</p>
                                <img src={ $hostRatings.hostCommunication.img } alt="Rating">
                            {:else}
                                <p>N/A</p>
                            {/if}
                        </div>
                    ...
                    <li>
                        ...
                        <div class="average-item-rating">
                            {#if $hostRatings.hostCheckIn.rating }
                                <p>{ $hostRatings.hostCheckIn.rating }</p>
                                <img src={ $hostRatings.hostCheckIn.img } alt="Rating">
                            {:else}
                                <p>N/A</p>
                            {/if}
                        </div>
                    ...
                    <li>
                        <h2>{ getLocalizedTextBySlug( 'compliance-with-rules-label', $languageTagStore ) }</h2>
                        <div class="average-item-rating">
                            {#if $hostRatings.hostCompliance.rating }
                                <p>{ $hostRatings.hostCompliance.rating }</p>
                                <img src={ $hostRatings.hostCompliance.img } alt="Rating">
                            {:else}
                                <p>N/A</p>
                            {/if}
                        </div>
